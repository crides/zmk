...
#define T_SRAD		    6			/* 4 us */
#define T_SRAD_MOTBR	T_SRAD
...
static int check_product_id(const struct device *dev)
{
    ...
    err = reg_read(dev, PMW3610_REG_NOT_PROD_ID, &product_id);
    if (err) {
        LOG_ERR("Cannot obtain inverse product id");
        return err;
    }

    if (product_id != (uint8_t) (~PMW3610_PRODUCT_ID)) {
        LOG_ERR("Incorrect inverse product id 0x%x (expecting 0x%x)!", product_id, (uint8_t) (~PMW3610_PRODUCT_ID));
        return -EIO;
    }
...
static int set_cpi(const struct device *dev, uint32_t cpi)
{
    ...
    /* set the cpi */
    uint8_t addr[] = {PMW3610_REG_SPI_PAGE, PAGE_OFF(PMW3610_REG_RES_STEP), PMW3610_REG_SPI_PAGE};
...
static int pmw3610_async_init_configure(const struct device *dev)
{
    ...
    if (!err) {
        err = set_downshift_time(dev, PMW3610_REG_RUN_DOWNSHIFT, CONFIG_PMW3610_RUN_DOWNSHIFT_TIME_MS);
    }

    if (!err) {
        err = set_downshift_time(dev, PMW3610_REG_REST1_DOWNSHIFT, CONFIG_PMW3610_REST1_DOWNSHIFT_TIME_MS);
    }
...
static int pmw3610_init(const struct device *dev)
{
    ...
    err = gpio_pin_configure_dt(&config->rst_gpio, GPIO_OUTPUT_INACTIVE);
    if (err) {
        LOG_ERR("set reset");
        return err;
    }
